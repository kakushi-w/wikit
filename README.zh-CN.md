...
-c, --config <路径>      配置文件（默认 ./config.json 或 $WIKICOMMA_CONFIG）
    ...
    --scheme <s>         wiki URL 协议：http 或 https（默认 https）
    --keep-removed       保留已从 sitemap 中消失的页面（不删除）
```

### 刷新评分与投票

普通扫描看不到评分/投票的变化（它们不会更新页面时间戳），本地存的评分会过期。
`--refresh-votes` 在备份后刷新它们，只重写页面元数据：
...
（带了参数以参数为准）。只有 HTTP 的站点把 `scheme` 设成 `http`（默认 `https`）。
`keep_removed` 会保留已从 wiki 删除的页面，不从本地归档中移除。
